...
from .igc_segment_importer_provider import IGCSegmentImporterProvider
# dialog
from .igc_segment_importer_dialog import IGCSegmentImporterDialog
# other
import os.path


# Based on http://www.qgistutorials.com/en/docs/building_a_python_plugin.html
class IGCSegmentImporter:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'igc_segment_importer_{}.qm'.format(QSettings().value('locale/userLocale')[0:2]))
        ...
        self.dlg = IGCSegmentImporterDialog()

        self.dlg.btnSelectFiles.clicked.connect(self.select_igc_files)
        ...
        self.igc_file_reader = IGCFileReader()

        # process
        self.provider = IGCSegmentImporterProvider()
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/IGCSegmentImporter/icon.svg'
        self.add_action(
            icon_path,
            text=self.tr(u'IGC Segment Importer'),
            callback=self.run,
    ...
    def select_igc_files(self):
        ...
        if len(self.igc_files) == 1:
            ...
            self.dlg.txtSelectedFiles.setText(str(len(self.igc_files)) + " files")

        # remember gpx directory path
        if len(self.igc_files) > 0:
            self.gpx_directory_default = os.path.abspath(self.igc_files[0])
        ...
        if len(self.igc_files) >= 1:
            self.igc_file_reader.get_table_data(self.igc_files[0])
            self.dlg.lblFeedback.setText(self.igc_file_reader.error_message)
            self.create_table()
    ...
    def select_output_directory(self):
        if self.output_directory is None:
            ...
            if os.path.exists(self.output_directory):
                ...
                if self.check_if_file_exists(self.output_directory, self.igc_files):
    ...
    def create_table(self):
        ...
        tm = AttributeTableModel(self.igc_file_reader.attribute_definitions, header, self)
        ...
        # set row height
        for row in range(len(self.igc_file_reader.attribute_definitions)):
